...
假设你要给一个班级的考试成绩排名，分数范围是 0 到 100 分。你不需要两两比较谁高谁低，只需要数一下：有几个 0 分、几个 1 分... 几个 100 分，然后按顺序把分数"倒出来"就行了。

**计数排序（Counting Sort）** 就是这个思路：不通过元素之间的比较来确定顺序，而是统计每个值出现的次数，再根据计数结果直接输出有序序列。它是**非比较排序**的代表，当数据范围有限时可以做到真正的 O(n) 时间复杂度。
...
前面所有基于比较的排序算法有一个理论下界：**O(n log n)**。这是因为 n 个元素有 n! 种排列，每次比较只能排除一半的可能性，至少需要 log₂(n!) = O(n log n) 次比较。

但计数排序**不做比较**，所以不受这个下界约束。它的代价是：需要额外的计数数组，且数据范围不能太大。
...
前缀和的含义：`prefix[2]=3` 表示值 <= 2 的元素有 3 个，所以最后一个 2 应该放在位置 2（下标 2）。反向遍历保证了相同值的元素保持原始相对顺序。
...
这就是为什么说计数排序适合"范围小的整数"。如果数据是 `[1, 1000000]`，要开一个百万大小的数组就不划算了。
...
- **怎么演示**：用两张流程图看清整体流程 + 前缀和稳定放置；用 `[4,2,2,8,3,3,1]` 看到"计数 -> 前缀和 -> 反向放置"三步走。
...
- **系列定位**：第一个非比较排序。下一篇桶排序进一步放宽条件——处理浮点数，用"分桶+桶内排序"的策略。
