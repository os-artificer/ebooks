...
你要给一堆身份证号排序。不需要两两比较大小，只需要这样做：先按最后一位数字分组（0~9 共 10 组），组内保持原始顺序收集回来；再按倒数第二位分组... 一直处理到最高位。整个过程下来，号码就全局有序了。

**基数排序（Radix Sort）** 就是这个思路：从最低位到最高位（或反过来），每一轮用一种稳定的排序算法（通常是计数排序）按当前位进行"分配-收集"。它也是非比较排序，对于固定位宽的整数可以达到 O(d × n) 的时间复杂度，d 是位数。
...
"像邮局分拣信件：先按最后一位邮编分，再按倒数第二位... 最后按第一位。" —— 关键在于**每轮必须使用稳定排序**，否则低位排好的顺序会被高位打乱。

### 为什么从低位开始（LSD）

这是最常见的基数排序变体（LSD Radix Sort）。从低位到高位处理的理由：
...
`getDigit(x, d)` 的实现：`(x / 10^(d-1)) % 10`。下面用流程图拆解逐位处理的主循环与单轮分配收集。
...
注意观察：每轮结束后数组并不是全局有序的（比如第 1 轮后 170 排到了最前面），但经过所有位数处理后最终有序。这就是"低位到高位"策略的正确性所在。
...
基数排序是**稳定的**，前提是每轮的内层排序（分配-收集）是稳定的。我们的实现中，同一位相同值的元素按原始顺序进入桶、按桶号顺序收集，因此保持了相对顺序。
...
相等值不存在（所有 key 都不同），但如果扩展测试加入相同值，它们的 tag 顺序会保持不变。
...
对比其他算法：如果 n=100 万，int32 最多 10 位，基数排序约 1000 万次操作；快排约 2000 万次比较（n log n）。在这个量级上基数排序往往更快。

### 空间复杂度

10 个桶 + 存放所有元素的副本 → **O(n + k) = O(n)**（k=10 是常数）

📌 小结算法：**时间 O(d×n)、空间 O(n)、稳定、非比较类、适用于固定位宽的整数/字符串**。
...
- **怎么演示**：用两张流程图看清逐位循环 + 单轮分配收集；用 `[170,45,75,90,802,24,2,66]` 的三轮输出看到"每轮只管一位、最终水到渠成"的过程。
...
- **系列定位**：十大经典排序的最后一位成员。至此，我们完整覆盖了 O(n²) 的简单排序（冒泡/选择/插入/希尔）、O(n log n) 的高效排序（归并/快排/堆）、以及 O(n) 的非比较排序（计数/桶/基数）。每种算法都有自己最适合的舞台。
