...
```text
// 饮料：只管一件事，把自己交出去
咖啡.接受(师傅)  =>  师傅.处理(咖啡)   // 转身去手冲台
酒.接受(师傅)    =>  师傅.处理(酒)     // 转身去醒酒
奶茶.接受(师傅)  =>  师傅.处理(奶茶)   // 转身去摇杯
```

新增一门手艺，等于店里多招一位师傅：

```text
直播带货员.处理(咖啡)  =>  喊"家人们看这个拉花"
直播带货员.处理(酒)    =>  喊"这是八二年的……"
直播带货员.处理(奶茶)  =>  喊"珍珠加双份"
```
...
同一个制作师，接到咖啡走手冲台，接到酒走醒酒器。关键就在 accept 那一递："我是谁"的信息顺着这一递，精准落到了对应的 visit 手法里，绝不走错工位。
