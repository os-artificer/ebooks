...
要点：Push 先挂链再改尾；_head->next 的 load 用 acquire，保证可见性；哑元节点让空队列时 Pop 直接返回 false。完整可运行 demo 见 `https://github.com/os-artificer/ebooks/tree/main/cpp/src/lock-free-queue/lock_free_queue_demo.cpp`（文件头部注释含编译与运行指令）。
